...
import { DirectionFacing, LeftRight } from './gameAtoms'
...
export const getNextTurnDirection = (
  currentDirection: DirectionFacing,
  leftRight: LeftRight,
  ...
  const clockwiseOrAntiDirectionList: DirectionFacing[] =
    leftRight === 'right' ? ['up', 'right', 'down', 'left'] : ['up', 'left', 'down', 'right']
  ...
  const nextDirectionIndex =
    currentDirectionIndex < filteredDirectionList.length - 1 ? currentDirectionIndex + 1 : 0
